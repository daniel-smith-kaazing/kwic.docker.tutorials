...
# REGENERATE if it all already exists.
REGENERATE=false
...
function usage()
{
  ...
  # echo "           --server-alt-names  \"DNS:example.com,DNS:*.example.com\""
}
...
function check_if_file_exists()
{
  ...
  then
    ...
    if [ "${REGENERATE}" == "true" ]
    then
      echo "It will be re-created because \"--regenerate\" is set to true. Just thought you should know."
    else
      echo "Stopping. Set \"--regenerate\" to \"true\" to continue when files exist. See usage with \"--help\""
      exit 1
    fi
  fi
}
...
function main()
{
  ...
  if [ "$REGENERATE" == "true" ]; then
      rm -rf ${OUTPUT_DIR}
  elif [[ -d "${OUTPUT_DIR}" && -f "${OUTPUT_DIR}/${CA_KEY}" && -f "${OUTPUT_DIR}/${CA_CERT}" ]]; then
      echo "${OUTPUT_DIR}, ${OUTPUT_DIR}/${CA_KEY} and ${OUTPUT_DIR}/${CA_CERT}  exist. Certs will not be re-generated."
      echo "HINT: To regenerate pass --regenerate. Good-bye!"
      exit 1
  fi
  mkdir -p ${OUTPUT_DIR}
